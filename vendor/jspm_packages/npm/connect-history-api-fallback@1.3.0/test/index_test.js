...
var historyApiFallback = require('../lib/index');
...
tests['should rewrite requests when the . rule is disabled'] = function(test) {
  req.url = 'js/app.js';
  middleware = historyApiFallback({disableDotRule: true});
  middleware(req, null, next);
...
tests['should accept html requests based on headers option'] = function(test) {
  req.headers.accept = '*/*';
  middleware = historyApiFallback({htmlAcceptHeaders: ['text/html', 'application/xhtml+xml']});
  middleware(req, null, next);
  test.equal(req.url, requestedUrl);
  test.ok(next.called);
  test.done();
