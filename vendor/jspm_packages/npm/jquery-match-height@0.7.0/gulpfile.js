...
(function(process) {
  ...
  gulp.task('release:push:github', function(callback) {
    return gulp.src(['CHANGELOG.md', buildDirectory + '/jquery.matchHeight-min.js', buildDirectory + '/jquery.matchHeight.js']).pipe(release({
      owner: 'liabru',
      repo: pkg.name,
      tag: pkg.version,
  ...
  gulp.task('build', function() {
    ...
    gulp.src('jquery.matchHeight.js').pipe(replace("jquery-match-height master", "jquery-match-height " + build.version)).pipe(replace("version = 'master'", "version = '" + build.version + "'")).pipe(gulp.dest(buildDirectory));
    return gulp.src('jquery.matchHeight.js').pipe(replace("version = 'master'", "version = '" + build.version + "'")).pipe(uglify({output: {max_line_len: 500}})).pipe(header(banner, {build: build})).pipe(rename({suffix: '-min'})).pipe(gulp.dest(buildDirectory));
  });
  gulp.task('lint', function() {
    return gulp.src(pkg.main).pipe(eslint()).pipe(eslint.format()).pipe(eslint.failAfterError());
  });
  var bump = function(options) {
    return gulp.src(['package.json', 'bower.json']).pipe(gulpBump(options)).pipe(gulp.dest('.'));
  };
  ...
  gulp.task('tag', function() {
    return gulp.src('package.json').pipe(tag({prefix: ''}));
  });
  gulp.task('changelog', function() {
    return gulp.src('CHANGELOG.md').pipe(changelog()).pipe(gulp.dest('.'));
  });
  ...
  gulp.task('test', function(done) {
    sequence('lint', 'serve:test', 'selenium', function() {
      ...
      gulp.src('test/conf/local.conf.js').pipe(webdriver({baseUrl: 'http://localhost:8000'})).on('error', function(err) {
        console.error(err);
        error = err;
      }).on('finish', finish);
    });
  ...
  gulp.task('test:cloud', ['lint', 'serve:test'], function(done) {
    ngrok.connect({
      ...
      gulp.src('test/conf/cloud.conf.js').pipe(webdriver({baseUrl: url})).on('finish', function(err) {
        if (server) {
          ngrok.disconnect();
          ngrok.kill();
  ...
  gulp.task('test:cloud:all', ['lint', 'serve:test'], function(done) {
    ngrok.connect({
      ...
      gutil.log('Tunnel started', url);
      gulp.src('test/conf/cloud-all.conf.js').pipe(webdriver({baseUrl: url})).on('finish', function(err) {
        if (server) {
          ngrok.disconnect();
          ngrok.kill();
          gutil.log('Tunnel stopped');
  ...
  var serve = function(isTest) {
    ...
    server = gulp.src('.').pipe(webserver({
      host: '0.0.0.0',
      livereload: !isTest,
  ...
  var banner = ['/*', '* <%= build.name %> <%= build.version %> by @liabru', '* <%= build.homepage %>', '* License <%= build.license %>', '*/', ''].join('\n');
