...
def release():
  ...
  prettyprint("Releasing Red Hat JBoss Data Grid Administration Console version %s from branch '%s'" % (version, branch), Levels.INFO)
  ...
  if not git.is_upstream_clone():
    proceed = input_with_default('This is not a clone of an %supstream%s Red Hat JBoss Data Grid Administration Console repository! Are you sure you want to proceed?' % (Colors.UNDERLINE, Colors.END), 'N')
